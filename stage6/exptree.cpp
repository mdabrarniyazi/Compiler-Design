...
if(root->nodeop==SLST) {eval(root->left); eval(root->right); return 0;}
else if(root->nodeop==EQ) {temp1=lookup(root->left->name);	
						if(temp1==NULL)	
						{
						...
						else
						{
							...
							*(temp1->bind)=eval(root->right);
							/*else
							{
								*(temp1->bind + eval(root->left->left))=eval(root->right); //for array	
							}*/	
						}}
else if(root->nodeop==IDD) {temp1=lookup(root->name);
							if(temp1==NULL)
							{
							...
							else
							{
								...
								/*else
									...
									return (*((temp1->bind)+eval(root->left)));
									}*/
							}}
else if(root->nodeop==REED){temp1=lookup(root->left->name);
						if(temp1==NULL)
						{
